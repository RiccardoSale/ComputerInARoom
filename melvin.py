...
def resume():
    ...
    init.sched.add_job(mapping.scan_map, 'interval', seconds=5, id='mapping', max_instances=1)
...
def go_and_scan(x, y, lens, id, coordinates=[], n_photo=0, is_multiple=False):
    ...
    if is_multiple:
        print("scan multiple")
        init.sched.add_job(lambda: objective_photo.photographer_multiple(lens, n_photo, coordinates), 'interval',seconds=x[1] - 1, id='pm',max_instances=1)
    else:
        print("scan single")
        init.sched.add_job(lambda: objective_photo.photographer(lens, id), 'interval', seconds=x[1], id='p',max_instances=1)
...
def resume_the_tour():
    ...
    if pixel_photo > dic_limit[lens]:  # multiple layer photos
        ...
        go_and_scan(res[init.vel_route][0], res[init.vel_route][1], lens, id, coordinates, n_photo, True)  
    else:  # ONE layer
        ...
        res = calculations.calculate_distance_time_fuel(final_x, final_y, t_to_target, curr_x, curr_y,
                                                        coord_x, coord_y, True)
        go_and_scan(res[init.vel_route][0], res[init.vel_route][1], lens, id)
    return
...
#start route to single type objective
def go_and_scan_single(x, y, lens, id):
    print("sono dentro scan single")
    if init.block is not True:
        pause()
    init.requests.put(init.url + "control",json={"state": "active", "camera_angle": "wide", "vel_x": x[0], "vel_y": y[0]})
    # manage the start cruise and deceleration job
